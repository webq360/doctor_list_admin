      ...
      {loading ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-5">
          {Array.from({ length: 12 }).map((_, i) => (
            <div key={i} className="bg-white rounded-2xl p-4 sm:p-5 border border-gray-100 animate-pulse">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-5">
          {statCards.map((c) => (
            <div key={c.key} className="bg-white rounded-2xl p-4 sm:p-5 border border-gray-100 hover:shadow-md transition-shadow">
              <div className="flex items-center justify-between mb-3 sm:mb-4">
                <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center" style={{ background: c.bg, color: c.color }}>
                  {c.icon}
                </div>
              </div>
              <p className="text-xl sm:text-2xl font-bold text-gray-800">{stats[c.key as keyof Stats]}</p>
              <p className="text-xs sm:text-sm text-gray-400 mt-0.5">{c.label}</p>
            </div>
          ))}
        </div>
      )}
