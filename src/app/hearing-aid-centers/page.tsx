...
function GalleryTab({ centerId }: { centerId: string }) {
  ...
  const fetchGallery = () =>
    api.get(`/hearing-aid-centers/${centerId}/gallery`).then((r) => setImages(r.data)).catch(() => {});
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.post(`/hearing-aid-centers/${centerId}/gallery`, {
        imageUrl,
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.put(`/hearing-aid-centers/${centerId}/gallery/${editItem._id}`, {
        imageUrl,
  ...
  const handleDelete = async (id: string) => {
    ...
    await api.delete(`/hearing-aid-centers/${centerId}/gallery/${id}`);
    fetchGallery();
  };
        ...
        <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
          ...
          <form onSubmit={handleAdd} className="p-5 space-y-4">
            ...
            <div>
              ...
              <input placeholder="e.g. Hearing Test Room, Equipment..." value={galleryForm.title}
    ...
    <>
      ...
      <div className="px-6 py-4">
        <div style={{ display: tab === 'services' ? 'block' : 'none' }}>
          <CenterServiceTab centerId={centerId} apiBase="/hearing-aid-centers" />
        </div>
...
const HEARING_SERVICES = ['Hearing Test', 'Hearing Aid Fitting', 'Hearing Aid Repair', 'Tinnitus Treatment', 'Cochlear Implant', 'Pediatric Audiology', 'Ear Wax Removal', 'Balance Testing'];

const emptyForm = { name: '', contact: '', address: '', division: '', district: '', upazila: '' };

interface HearingAidCenter {
  _id: string;
  name: string;
  contact: string;
  address: string;
...
function HearingAidModal({ title, onClose, onSubmit, data, setData, isEdit, form, setForm, selectedServices, setSelectedServices, loading, error }: ModalProps) {
          ...
          <form onSubmit={onSubmit} className="px-6 py-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label className={labelCls}>Center Name<span className="text-red-400 ml-0.5">*</span></label>
                <input placeholder="Hearing Aid Center Name" value={isEdit ? data.name : form.name}
...
export default function HearingAidCentersPage() {
  const [centers, setCenters] = useState<HearingAidCenter[]>([]);
  ...
  useEffect(() => {
    api.get('/hearing-aid-centers').then((r) => setCenters(r.data)).catch(() => {});
  }, []);
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      const { data } = await api.post('/hearing-aid-centers', { ...form, services: selectedServices });
      setCenters((p) => [data, ...p]);
  ...
  const handleEdit = async (e: React.FormEvent) => {
    ...
    setLoading(true); setError('');
    try {
      const { data } = await api.put(`/hearing-aid-centers/${editCenter._id}`, editCenter);
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Delete this hearing aid center?')) return;
    await api.delete(`/hearing-aid-centers/${id}`);
    setCenters((p) => p.filter((c) => c._id !== id));
  };

  const toggleActive = async (c: HearingAidCenter) => {
    const { data } = await api.put(`/hearing-aid-centers/${c._id}`, { isActive: !c.isActive });
    ...
    <AdminLayout title="Hearing Aid Centers" action={addBtn}>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {centers.length === 0 && (
              <tr><td colSpan={6} className="px-5 py-10 text-center text-gray-300 text-sm">No hearing aid centers found</td></tr>
            )}
            {centers.map((c) => (
      ...
      {editCenter && <HearingAidModal title="Edit Hearing Aid Center" onClose={() => setEditCenter(null)} onSubmit={handleEdit} data={editCenter} setData={setEditCenter} isEdit={true} form={form} setForm={setForm} selectedServices={selectedServices} setSelectedServices={setSelectedServices} loading={loading} error={error} />}
