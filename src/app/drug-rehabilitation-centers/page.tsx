...
const API_BASE = '/drug-rehabilitation-centers';
const REHAB_SERVICES = ['Detoxification', 'Inpatient Rehab', 'Outpatient Rehab', 'Counseling', 'Group Therapy', 'Family Therapy', 'Aftercare Support', 'Relapse Prevention', 'Psychiatric Support', 'Vocational Training'];
const emptyForm = { name: '', contact: '', address: '', division: '', district: '', upazila: '' };
const emptyGalleryForm = { title: '', description: '' };

interface DrugRehabCenter {
  _id: string;
  name: string;
  contact: string;
  address: string;
...
function GalleryTab({ centerId }: { centerId: string }) {
  ...
  const fetchGallery = () =>
    api.get(`${API_BASE}/${centerId}/gallery`).then((r) => setImages(r.data)).catch(() => {});
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.post(`${API_BASE}/${centerId}/gallery`, {
        imageUrl,
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.put(`${API_BASE}/${centerId}/gallery/${editItem._id}`, {
        imageUrl,
  ...
  const handleDelete = async (id: string) => {
    ...
    await api.delete(`${API_BASE}/${centerId}/gallery/${id}`);
    fetchGallery();
  };
  ...
  const openEdit = (img: any) => {
    ...
    setEditImageFile(null); setEditImagePreview(''); setShowForm(false);
  };
        ...
        <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
          ...
          <form onSubmit={handleAdd} className="p-5 space-y-4">
            ...
            <div>
              ...
              <input placeholder="e.g. Therapy Room, Facility..." value={galleryForm.title}
      ...
      <div className="flex gap-1 px-6 pt-4">
        {(['services', 'gallery'] as const).map((t) => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-4 py-2 rounded-xl text-sm font-medium capitalize transition-colors ${tab === t ? 'text-white' : 'text-gray-500 hover:bg-gray-50'}`}
            style={tab === t ? { background: '#2B3EE6' } : {}}>
            {t === 'services' ? 'Services' : 'Gallery'}
          </button>
        ))}
      </div>
      <div className="px-6 py-4">
        {tab === 'services' && <CenterServiceTab centerId={centerId} apiBase={API_BASE} />}
...
function DrugRehabModal({ title, onClose, onSubmit, data, setData, isEdit, form, setForm, selectedServices, setSelectedServices, loading, error }: ModalProps) {
          ...
          <form onSubmit={onSubmit} className="px-6 py-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label className={labelCls}>Center Name<span className="text-red-400 ml-0.5">*</span></label>
                <input placeholder="Rehab Center Name" value={isEdit ? data.name : form.name}
...
// ── Main Page ──
export default function DrugRehabilitationCentersPage() {
  const [centers, setCenters] = useState<DrugRehabCenter[]>([]);
  ...
  useEffect(() => {
    api.get(API_BASE).then((r) => setCenters(r.data)).catch(() => {});
  }, []);

  const handleAdd = async (e: React.FormEvent) => {
    e.preventDefault(); setLoading(true); setError('');
    try {
      const { data } = await api.post(API_BASE, { ...form, services: selectedServices });
      setCenters((p) => [data, ...p]);
  ...
  const handleEdit = async (e: React.FormEvent) => {
    e.preventDefault(); setLoading(true); setError('');
    try {
      const { data } = await api.put(`${API_BASE}/${editCenter._id}`, editCenter);
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Delete this rehab center?')) return;
    await api.delete(`${API_BASE}/${id}`);
    setCenters((p) => p.filter((c) => c._id !== id));
  };

  const toggleActive = async (c: DrugRehabCenter) => {
    const { data } = await api.put(`${API_BASE}/${c._id}`, { isActive: !c.isActive });
    ...
    <AdminLayout title="Drug Rehabilitation Centers" action={addBtn}>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        ...
        <table className="w-full text-sm" style={{ minWidth: '650px' }}>
          ...
          <tbody>
            {centers.length === 0 && (
              <tr><td colSpan={6} className="px-5 py-10 text-center text-gray-300 text-sm">No rehabilitation centers found</td></tr>
            )}
            {centers.map((c) => (
              <tr key={c._id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                ...
                <td className="px-5 py-3.5">
                  <div className="flex flex-wrap gap-1">
                    {c.services.slice(0, 3).map((s) => (
                      <span key={s} className="px-2 py-0.5 rounded-md text-xs font-medium bg-purple-50 text-purple-600">{s}</span>
                    ))}
                ...
                <td className="px-5 py-3.5">
                  <div className="flex items-center gap-1">
                    ...
                    {/* Edit */}
                    <button onClick={() => { setEditCenter({ ...c }); setError(''); }} title="Edit"
      ...
      {/* Add Modal */}
      {showModal && (
        <DrugRehabModal title="Add Rehab Center" onClose={() => setShowModal(false)} onSubmit={handleAdd}
          data={form} setData={setForm} isEdit={false} form={form} setForm={setForm}
          selectedServices={selectedServices} setSelectedServices={setSelectedServices} loading={loading} error={error} />
      )}

      {/* Edit Modal */}
      {editCenter && (
        <DrugRehabModal title="Edit Rehab Center" onClose={() => setEditCenter(null)} onSubmit={handleEdit}
          data={editCenter} setData={setEditCenter} isEdit={true} form={form} setForm={setForm}
          selectedServices={selectedServices} setSelectedServices={setSelectedServices} loading={loading} error={error} />
      )}

      {/* View Modal with 2 Tabs */}
      {viewCenter && (
        <div className="fixed inset-0 z-50 overflow-y-auto" style={{ background: 'rgba(0,0,0,0.4)' }}>
          <div className="min-h-full flex items-start justify-center p-6 py-10">
            <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl">
